...
export class NotificationsService {
  ...
  getNotifications(): Observable<NotificationModel[]> {
    return this.http.get<NotificationModel[]>(`${ENDPOINT}/${API_NOTIFICATIONS_URL}`);
  }
  ...
  updateNotificationStatus(notification: NotificationModel, status: NotificationStatus) {
    ...
    return this.http.put<NotificationModel>(
      `${ENDPOINT}/${API_NOTIFICATIONS_URL}/${notification.id}`,
      notification,
      { headers: httpHeaders }
    );
  }
  ...
  deleteNotification(id: number) {
    const url = `${ENDPOINT}/${API_NOTIFICATIONS_URL}/${id}`;
